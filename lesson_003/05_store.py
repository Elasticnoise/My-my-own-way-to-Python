...
#     all_quantity = 0
#     total_price = 0
#     product = store[goods[key]]
#     for items in product:
#         all_quantity += (items['quantity'])
#         total_price += (items['quantity']) * (items['price'])
#     print(key, '-', all_quantity, 'шт, стоймостью', total_price, 'руб')


# Используйте силу распаковки словаря!
for name, code in goods.items():
    ...
    for items in product:
        all_quantity += (items['quantity'])
        total_price += (items['quantity']) * (items['price'])
    print(name, '-', all_quantity, 'шт, стоймостью', total_price, 'руб')
